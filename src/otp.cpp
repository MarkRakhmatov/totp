...
{
    char *nK = static_cast<char*>(calloc (strlen (K) + 1, 1));
    if (nK == NULL) {
              ...
              whmac_handle_t *hd)
{
    ...
    err = static_cast<cotp_error_t>(whmac_setkey (hd, secret, secret_len));
    if (err) {
    ...
    size_t dlen = whmac_getlen (hd);
    unsigned char *hmac = static_cast<unsigned char*>(calloc (dlen, 1));
    if (hmac == NULL) {
...
finalize (int digits_length,
          int tk)
{
    char *token = static_cast<char*>(calloc (digits_length + 1, 1));
    if (token == NULL) {
        return NULL;
    }
