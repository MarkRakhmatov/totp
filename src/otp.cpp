...
static void secure_memzero(void *ptr, size_t n) {
  auto *vptr = (volatile unsigned char *)ptr;
  while ((n--) != 0U) {
    *vptr++ = 0;
  }
}
...
{
  char *nK = (char*)calloc (strlen (K) + 1, 1);
  if (nK == NULL) {
    fprintf (stderr, "Error during memory allocation\n");
    return nK;
  }
  for (int i = 0, j = 0; K[i] != '\0'; i++) {
    if (int(K[i]) <= -1 || (!isalnum(K[i]) && K[i] != '='&& K[i] != ' ')) {
      free(nK);
      return NULL;
    }
    if (K[i] != ' ') {
             ...
             whmac_handle_t *hd)
{
  ...
  cotp_error_t err = (cotp_error_t)whmac_setkey (hd, secret.data(), secret.size());
  if (err) {
  ...
  size_t dlen = whmac_getlen (hd);
  unsigned char *hmac = (uchar*)calloc (dlen, 1);
  if (hmac == NULL) {
